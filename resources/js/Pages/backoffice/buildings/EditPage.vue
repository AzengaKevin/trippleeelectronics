...
const breadcrumbs = [
    ...
    { title: props.building.name, href: route('backoffice.buildings.show', props.building.id) },
    { title: 'Edit', href: null },
];

const form = useForm({
    property: props.building.property_id,
    name: props.building.name,
    code: props.building.code,
    address: props.building.address,
    active: props.building.active,
});

const { showFeedbackSwal, showInertiaErrorsSwal } = useSwal();

const submitForm = () => {
    form.put(route('backoffice.buildings.update', props.building.id), {
        onError: (errors) => {
...
<template>
    ...
    <BackofficeLayout :breadcrumbs="breadcrumbs" title="Edit Building">
