...
const breadcrumbs = [
    ...
    { title: 'Purchases', href: route('backoffice.purchases.index') },
    { title: `Purchase # ${props.purchase?.reference}`, href: route('backoffice.purchases.show', [props.purchase?.id]) },
...
const submit = () => {
    form.transform((data) => {
        ...
        return data;
    }).put(route('backoffice.purchases.update', [props.purchase.id]), {
        onError: (errors) => {
...
<template>
    ...
    <teleport to="body">
        ...
        <dialog ref="purchaseSupplierDialog" class="modal">
            <select-client-component :client="purchase.supplier" @client-updated="handleClientUpdated" @close-dialog="closePurchaseSupplierDialog" />
        </dialog>
    </teleport>
</template>
