...
const breadcrumbs = [
    ...
    {
        title: `# ${props.suspension.id}`,
        href: route('backoffice.suspensions.show', [props.suspension.id]),
    },
    {
        title: 'Edit',
        href: null,
...
const form = useForm({
    employee: props.suspension.employee_id,
    from: props.suspension?.from,
    to: props.suspension?.to,
    reason: props.suspension?.reason,
});

const submit = () => {
    form.put(route('backoffice.suspensions.update', [props.suspension.id]), {
...
<template>
    <Head>
        <title>Update Suspension</title>
    </Head>

    <BackofficeLayout :breadcrumbs="breadcrumbs" title="Update Suspension">
        <div class="space-y-4">
