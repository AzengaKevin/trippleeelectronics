...
const breadcrumbs = [
    ...
    {
        title: 'Employment',
        href: route('backoffice.employment.show'),
    },
    {
        title: 'Edit',
        href: null,
...
const addDocument = () => {
    form.documents.push({ ...documentBlueprint });
};
...
const submit = () => {
    form.post(route('backoffice.employment.update'), {
        onSuccess: () => {
            form.reset();
        },
        onError: (errors) => {
...
<template>
    <Head>
        <title>Edit Employment Details</title>
    </Head>

    <BackofficeLayout :breadcrumbs="breadcrumbs" title="Edit Employment Details">
        <div class="space-y-4">
                    ...
                    <form @submit.prevent="submit" class="grid grid-cols-1 gap-4" novalidate>
                        ...
                        <fieldset class="space-y-2">
                            ...
                            <div class="overflow-x-auto">
                                <table class="rounded-box table-xs table w-full">
                                    <thead>
                                        <tr>
                                            <th>Type</th>
                                            <th>File</th>
                                    ...
                                    <tbody>
                                        <template v-if="form.documents.length">
                                            <tr v-for="(document, index) in form.documents">
                                                <td>
                                                    <select v-model="document.type" class="select select-sm select-bordered w-full">
                                                        <option value="">Select Option</option>
                                                        <option v-for="docType in documentTypes" :value="docType.value">
                                                            {{ docType.label }}
                                                        </option>
                                                    </select>
                                                </td>
                                                <td>
                                                    <input
                                                        type="file"
                                                        @change="document.file = $event.target.files[0]"
                                    ...
                                    <tfoot>
                                        <tr>
                                            <td>
                                                <button type="button" @click="addDocument" class="btn btn-sm btn-primary btn-outline">
