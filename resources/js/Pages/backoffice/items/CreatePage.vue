...
watch(
    ...
    (newValue) => {
        if (newValue) {
            axios
                .get(route('api.products.index', { query: newValue }))
                .then((response) => {
                    similarItems.value = response.data.data;
                })
                .catch((error) => {
                    console.error('Error checking item name:', error);
                    form.errors.name = 'An error occurred while checking the item name.';
                });
        }
    },
);
...
const createItem = () => {
    form.post(route('backoffice.items.store'), {
        onSuccess: () => {
            form.reset();
        },
        onError: (errors) => {
...
<template>
    ...
    <BackofficeLayout :breadcrumbs="breadcrumbs" title="Create Item">
        <div class="space-y-4">
            <div class="card bg-base-100 shadow">
                <div class="card-body lg-p-6 p-2 sm:p-4">
                    <form @submit.prevent="createItem" class="grid grid-cols-1 gap-4" novalidate>
                        <!-- Item Name -->
                        <div class="space-y-2">
                            ...
                            <ul v-if="similarItems.length" class="space-y-3">
                                <li v-for="item in similarItems">{{ item.name }}</li>
                            </ul>
