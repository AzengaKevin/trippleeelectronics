...
const breadcrumbs = [
    ...
    },
    {
        ...
        href: route('backoffice.users.show', [props.user.id]),
    },
    {
        title: 'Edit',
        href: null,
...
const form = useForm({
    ...
    stores: props.user.stores?.map((s) => s.id),
});

const updateUser = () => {
    form.patch(route('backoffice.users.update', [props.user.id]), {
        onSuccess: () => {
            form.reset();
        },
        onError: (errors) => {
...
<template>
    ...
    <BackofficeLayout :breadcrumbs="breadcrumbs" title="Update User">
        <div class="space-y-4">
            <div class="card bg-base-100 shadow">
                <div class="card-body lg-p-6 p-2 sm:p-4">
                    <form @submit.prevent="updateUser" class="grid grid-cols-1 gap-4" novalidate>
                        <div class="space-y-2">
