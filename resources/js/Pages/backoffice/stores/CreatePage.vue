...
const form = useForm({
    ...
    address: '',
    location: '',
    payment_methods: [{ ...paymentMethodBlueprint }],
});
...
<template>
    ...
    <BackofficeLayout :breadcrumbs="breadcrumbs" title="Create Store">
        <div class="space-y-4">
            <ul v-if="form.errors?.length" class="text-error space-y-2 text-sm">
                <li v-for="(error, index) in form.errors" :key="index" class="text-error">
                    {{ error }}
                </li>
            </ul>
            ...
            <div class="card bg-base-100 shadow">
                <div class="card-body lg-p-6 p-2 sm:p-4">
                    <form @submit.prevent="createStore" class="grid grid-cols-1 gap-4" novalidate>
                        <div class="space-y-2">
